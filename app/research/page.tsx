...
export default function ResearchPage() {
  ...
  const allTags = useMemo(
    () => Array.from(new Set(research.flatMap((r) => r.tags))).sort(),
    []
  );
  const years = useMemo(
    () =>
      Array.from(
        new Set(
          research.map((r) => new Date(r.date).getFullYear().toString())
        )
    ...
    return research
      .filter((item) => {
        ...
        const matchesQuery = [item.title, item.venue, ...item.tags]
          ...
          .toLowerCase()
          .includes(q);
        const matchesTags = selectedTags.every((t) => item.tags.includes(t));
